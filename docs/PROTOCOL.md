...
**Experimental, more so than the rest of this document** - `pairing.mjs`'s own top-of-file comment is the source of truth for exactly which parts are precisely confirmed against AOSP/BoringSSL source (the curve, mask constants, password-scalar derivation, and key-derivation transcript order all are, cross-checked across multiple independent reads) versus a small number of best-effort placeholders for details that couldn't be pinned to the exact byte from public source alone (the `PairingPacketHeader` version byte, the AES key's HKDF `info` string, and the per-direction nonce counter's exact layout) - distinct from the general "not yet validated against a real device" caveat, since local testing alone can't confirm a literal byte value is right. See [#1](https://github.com/CLDMV/droidsock/issues/1).
