...
- **Shell-metacharacter injection in `devices.mjs`'s convenience shortcuts** - `ls()`, `getprop()`, `screenshot()`, `keypress()`, and `launchApp()` interpolated caller-supplied values into shell commands with double quotes or no quoting at all, the same class of gap v1.2.0 closed in `files.mjs`. All five now go through the shared `utils.quoteShellArg()` helper.
