...
export async function pair(host, port, pairingCode, options = {}) {
	...
	return new Promise((resolve, reject) => {
		...
		const socket = tls.connect({
			...
			minVersion: "TLSv1.3",
			maxVersion: "TLSv1.3",
			// AOSP's pairing server presents an ephemeral, unpinned cert -
			// trust here comes from the shared pairing code via SPAKE2, not
			// from certificate validation (confirmed: pairing_connection.cpp
			// sets a cert-verify callback that unconditionally accepts).
